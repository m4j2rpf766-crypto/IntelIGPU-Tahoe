...
static void reimsProjectRP(const uint8_t *native,uint8_t old[0x108],BOOL coverage,BOOL one){
 ...
 // Tahoe logicOp starts at bit32 (32-bit bitfield allocation boundary),
 // not bit29. TGL consumes logicOp at27 and enable at31 (0x7abc/0x7b40).
 // Preserve low fields through alphaTestEnabled; map logicOp and the three
 // following legacy bits separately. New-only tail fields remain excluded.
 flags=((flags>>2)&(((UINT64_C(1)<<27)-1)&~UINT64_C(3))) |
       (((flags>>32)&UINT64_C(0x7f))<<27) | (coverage?1:0) | (one?2:0);
