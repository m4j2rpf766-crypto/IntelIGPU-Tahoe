...
def verify_bundle(run):
    for name,expected in HASHES.items():
        require(hashlib.sha256((BUNDLE/name).read_bytes()).hexdigest()==expected,'VideoDiscovery package changed: '+name)
    run(['codesign','--verify','--deep','--strict',BUNDLE])
    info=plistlib.loads((BUNDLE/'Contents/Info.plist').read_bytes())
    require(info['CFBundleIdentifier']==IDENTIFIER and info['CFBundleVersion']==RECEIPT['version'],'Unexpected VideoDiscovery identity')
    personalities=info['IOKitPersonalities'];require(len(personalities)==1,'Expected one VideoDiscovery personality')
...
def check_loaded(loaded):
    ...
    if lines:require(len(lines)==1 and UUID in lines[0].upper().replace('-',''),'Unexpected loaded VideoDiscovery version; do not replace a running publisher')
    return bool(lines)
def ensure_video(run,registry,objects,logdir):
    ...
    require(len(accelerators)==1 and accelerators[0].get('IOServiceState',0)&2,'Video discovery requires an already-published accelerator; no display activation was attempted')
    ...
    if needed:
        ...
        require(check_loaded(run(['kmutil','showloaded']).decode()),'VideoDiscovery did not load; complete normal macOS approval, then rerun igpu-start')
    deadline=time.monotonic()+5
    while not published(pci,objects,properties):
        require(time.monotonic()<deadline,'VideoDiscovery loaded but did not publish expected properties; inspect evidence, do not repeat display commit')
