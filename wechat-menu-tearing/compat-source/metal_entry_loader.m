#import "../../app-blank-candidate/stride-source/metal_entries.m"
#include <mach-o/loader.h>
#include <mach-o/dyld.h>
#import "../../app-blank-candidate/logic-source/pipeline_descriptor_compat.m"
#import "../../app-blank-candidate/texture_sync_compat.m"

#import "../../jianying-startup-candidate/final-source/dynamic_library_compat.m"
#import "menu_stencil_compat.m"
...
BOOL ReimsInstallMetalEntryCandidate(const char *path){
 @synchronized([ReimsTahoeMetalDevice class]){
  ...
  if(result&&(!strcmp(getprogname(),"WeChat")||!strcmp(getprogname(),"iconservicesagent")))result=reimsInstallTextureSync(nativeInfo.dli_fbase);
  if(result)result=reimsInstallDynamicLibraryCompat(device);
  if(result)result=reimsInstallMenuStencilCompat(nativeInfo.dli_fbase);
